...
class FaceRecognizer:
    ...
    def process(self, stream):
        ...
        while stream.isOpened():
            ...
            # RFID check (non-blocking read with try-except)
            rfid_name = None
            try:
                id, text = reader.read_no_block()  # you might need to implement read_no_block, else use read() carefully
                if id in self.rfid_tags:
                    rfid_name = self.rfid_tags[id]
                    self.attendance(rfid_name)
                    known_face_detected = True
                    cv2.putText(img_rd, f"RFID: {rfid_name}", (10, 30), self.font, 0.7, (0, 255, 0), 2)
                    print(f"RFID authorized: {rfid_name}")
            except Exception:
                # Ignore no card or read errors
                pass

            current_time = time.time()

            # Relay control: ON for 10 seconds if either face or RFID authorized
            ...
            cv2.imshow("Face & RFID Recognizer", img_rd)
